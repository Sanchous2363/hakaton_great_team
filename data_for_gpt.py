...
def regestration_for_people(user_id, role, content, tokens):
    connection = sqlite3.connect('users.db')
    cur = connection.cursor()
    first_info = (user_id, role, content, tokens)
    cur.execute(
        f'INSERT INTO users3 (user_id, role, content, tokens) VALUES (?, ?, ?, ?);',
        first_info)
    connection.commit()
    cur.close()

def regestration_for_assistent(user_id, role, content, tokens):
    connection = sqlite3.connect('users.db')
    cur = connection.cursor()
    first_info = (user_id, role, content, tokens)
    cur.execute(
